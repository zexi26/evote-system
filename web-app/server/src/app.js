...
app.post('/registerVoter', async (req, res) => {
  ...
  let response = await network.registerVoter(voterId, req.body.nationId, req.body.firstName, req.body.lastName, req.body.email, req.body.number);
  ...
  if (response.error) {
    ...
    if (invokeResponse.error) {
      ...
      parsedResponse += '. Use game username to login above.';
      res.send(parsedResponse);
...
app.post('/validateVoter', async (req, res) => {
  ...
  if (invokeResponse.error) {
    ...
    if (parsedResponse.ballotCast) {
      let response = {};
      response.error = 'Sorry you have already casted a vote, double-voting is not allowed!';
      res.send(response);
    }
...
app.post('/Checkpick', async (req, res) => {
  console.log('req.body: ');
  console.log(req.body);
