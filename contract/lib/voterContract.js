...
const eventDataPath = path.join(process.cwd(), './lib/data/eventData.json');
const eventDataJson = fs.readFileSync(eventDataPath, 'utf8');
const eventData = JSON.parse(eventDataJson);

// connect to the pres election file
const ballotDataPath = path.join(process.cwd(), './lib/data/player.json');
...
class MyAssetContract extends Contract {
  ...
  async init(ctx) {
    ...
    if (currElections.length === 0) {
      ...
      let eventStartDate = await new Date(2020, 8, 15);
      let eventEndDate = await new Date(2020, 8, 18);

      //create the election
      election = await new Election(eventData.eventName, eventData.eventCountry,
        eventData.eventYear, eventStartDate, eventEndDate);
    ...
    let gVotable = await new VotableItem(ctx, 'Republican', ballotData.gBrief);
    let cVotable = await new VotableItem(ctx, 'Democrat', ballotData.cBrief);
    let fVotable = await new VotableItem(ctx, 'Green', ballotData.fBrief);
    let bVotable = await new VotableItem(ctx, 'Independent', ballotData.bBrief);
    let nVotable = await new VotableItem(ctx, 'Libertarian', ballotData.nBrief);

    //populate choices array so that the ballots can have all of these choices 
    votableItems.push(gVotable);
    votableItems.push(cVotable);
    votableItems.push(fVotable);
    votableItems.push(bVotable);
    votableItems.push(nVotable);
  ...
  async createVoter(ctx, args) {
    ...
    //create a new voter
    let newVoter = await new Voter(args.voterId, args.nationId, args.firstName, args.lastName, args.email, args.number);
    ...
    let response = `Voter with game username ${newVoter.voterId} is updated in the voting state`;
    return response;
  }
  ...
  async deleteMyAsset(ctx, myAssetId) {
    ...
    if (!exists) {
      throw new Error(`The voter with game username ${myAssetId} does not exist`);
    }
  ...
  async readMyAsset(ctx, myAssetId) {
    ...
    if (!exists) {
      // throw new Error(`The voter with game user name ${myAssetId} does not exist`);
      let response = {};
      response.error = `The voter with game user name ${myAssetId} does not exist`;
      return response;
    }
  ...
  async castVote(ctx, args) {
    ...
    if (electionExists) {
      ...
      if (voter.ballotCast) {
        let response = {};
        response.error = 'This voter has already cast the vote!';
        return response;
      }

      //check the date of the election, to make sure the election is still open
      let currentTime = await new Date(2020, 8, 15);
      ...
      if (parsedCurrentTime >= electionStart && parsedCurrentTime < electionEnd) {
        ...
        let response = {};
        response.error = 'The event is not open now!';
        return response;
      }

    } else {
      let response = {};
      response.error = 'The event or the voter does not exist!';
      return response;
    }
  }
